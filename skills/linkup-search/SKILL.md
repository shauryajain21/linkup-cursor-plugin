...
- Conversational chatbot where low latency is important: keep prompts simple, keyword style, standard depth
- Deep researcher: more detailed, leverage scraping, deep depth
...
## 5. Using the Fetch Command

When your agent already knows the exact URL, use `linkup fetch` instead of `linkup search`. It's faster, cheaper, and purpose-built for single-page extraction.
...
Use date ranges to limit results to a time window:

```
Query: "Find news about Anthropic product launches"
(limit to specific date range as needed)
```

Use domain filtering to focus on specific sources:

```
Query: "Find Tesla's latest quarterly earnings data"
(focus on tesla.com, sec.gov)
```
...
The Linkup CLI provides direct terminal access to search and fetch capabilities.
...
| Command | Description |
|---------|-------------|
| `linkup search "query"` | Standard depth search with AI synthesis |
| `linkup search "query" --depth deep` | Deep research mode |
| `linkup search "query" --output searchResults` | Raw results without synthesis |
| `linkup fetch "url"` | Extract content from a URL |
| `linkup config` | View current configuration |

### Options

| Flag | Short | Values | Default |
|------|-------|--------|---------|
| `--depth` | `-d` | fast, standard, deep | standard |
| `--output` | `-o` | sourcedAnswer, searchResults | sourcedAnswer |
...
```
STANDARD:  Parallel searches ✓  Scrape one provided URL ✓  Scrape multiple URLs ✗  Chain search→scrape ✗
DEEP:      Iterative searches ✓  Scrape multiple URLs ✓   Chain search→scrape ✓
UNCERTAIN: Default to deep.
OUTPUT:    searchResults (raw sources)  |  sourcedAnswer (natural language)  |  structured (JSON schema)
FETCH:     Single known URL → linkup fetch "url"
